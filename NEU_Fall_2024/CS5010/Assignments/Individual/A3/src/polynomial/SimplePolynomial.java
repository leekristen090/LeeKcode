...
/**
 * This class represents a SimplePolynomial which implements the Polynomial interface which
 ...
 * power. Polynomials are one-variable if all their terms contain only one variable.
 */
public class SimplePolynomial implements Polynomial {
  ...
  public Polynomial derivative() {
    ...
    for (int i = 1; i < numbers.size(); i++) {
      ...
      //int newPower = i - 1;
      result.addTerm(numbers.get(i) * i, i - 1 );
      //result.addTerm(thisCoeff * i, newPower);
      //if (newPower == 0) {result.addTerm(thisCoeff * i,newPower);}
      //if (newPower != 0) {result.addTerm(thisCoeff * i, newPower);}
    }
    return result;
  }
  ...
  public boolean equals(Object o) {
    ...
    //int largest = Math.max(this.getDegree(), other.getDegree());
