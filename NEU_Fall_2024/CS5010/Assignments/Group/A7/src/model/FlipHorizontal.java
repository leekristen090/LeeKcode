...
/**
 * The {@code flipHorizontal} class implements the {@code ImageProcessing} interface
 * to flip an image horizontally.
 */
public class FlipHorizontal implements ImageProcessing {

  /**
   * Processes the specified image by flipping it horizontally.
   *
   ...
   * @return an array containing the horizontally flipped {@code Image}.
  ...
  public Image[] process(String[] args, ImageDB images)
    ...
    if (args.length != 3) {
      throw new IllegalArgumentException("flipHorizontal command requires an image name "
              + "and output");
    }
    Image obj = images.get(args[1]);
    int width = obj.getWidth();
    int height = obj.getHeight();
    Image horizontalImage = obj.createImage(width, height);
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        horizontalImage.setPixel(width - 1 - x, y, pixel);
      }
    }
    return new Image[]{horizontalImage};
  }
