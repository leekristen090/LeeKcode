...
public class BSTElementNode<T extends Comparable<T>> implements BSTNode<T> {
  ...
  /**
   * Constructor for BSTElementNode.
   * @param data the data
   * @param left data to the left
   * @param right data to the right
   */
  ...
  /**
   * Traverse the tree rooted at this node in preorder (NLR) and apply the consumer at each piece
   * of data.
  ...
  /**
   * Traverse the tree rooted at this node in postorder (LRN) and apply the consumer at each piece
   * of data.
   * @param consumer the function object to be applied at each piece of data
   */
  @Override
  ...
  public BSTNode<T> copy() {
    // same order os postorder (LRN)
    BSTNode<T> leftCopy = this.left.copy();
    BSTNode<T> rightCopy = this.right.copy();
    return new BSTElementNode<>(this.data, leftCopy, rightCopy);
    // return null;
  }
  ...
  /**
   * Determine if this element node is the same (content and structure wise) as an element node.
   * @param other the other element node
   * @return return true if element nodes are same, false otherwise
   */
  @Override
  public boolean sameElement(BSTElementNode<T> other) {
    ...
    return this.left.same(other.left) && this.right.same(other.right);
  }

  /**
   * Determine if an element node is the same as another empty node.
   * @param other the other empty node
   * @return false bc element node can't be same as empty node
   */
  @Override
